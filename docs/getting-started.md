...
>
> ⚡️ 想要“零配置”体验搜索意图场景？执行 `python scripts/pipeline.py --preset search-intent-lora` 会自动从魔搭拉取搜索意图数据集，在 Qwen/Qwen3-0.6B 上进行 MPS LoRA 训练，并优先使用本地 Ollama `qwen3:8b` 进行自动评测；若 Ollama 未安装则自动回退至 0.6B。
>
